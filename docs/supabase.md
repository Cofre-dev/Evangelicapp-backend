...
Hoy Supabase se usa solo como host de Postgres (confirmado: no hay `@supabase/*` en `package.json` ni referencias a Supabase en `backend/src`). Este documento evalúa activar las otras cuatro piezas del plan Pro ($25/mes) para que ese pago se justifique, con impacto en el usuario, pasos técnicos, riesgo y dependencias de cada una — sin escribir código todavía.

## Bloqueadores antes de ejecutar cualquier ítem

1. **MCP de Supabase sin autenticar.** Quedó agregado en scope local pero el login OAuth nunca se completó (ver memoria `supabase-mcp-pending-auth`). Nada de lo de abajo se puede ejecutar contra la cuenta real hasta correr `/mcp` → `supabase` → login.
2. **Proyecto de Supabase pausado.** Confirmar que está reactivado (plan free lo pausó y causó la caída del 2026-08-07) antes de dar por buena cualquier prueba contra él.
...
**Recomendación:** no activar Realtime para iglesias reales (más allá de una de prueba) hasta que la Fase 8 (RLS) esté lista — es la única forma de tener una garantía real, no solo "el frontend filtra bien", de que una iglesia no reciba eventos de otra.
...
7. Decidir esquema de cookies/CSRF: hoy es cookie httpOnly propia + CSRF double-submit; Supabase recomienda `@supabase/ssr` para manejo de cookies, que tiene su propio patrón.
8. Migrar el flujo de invitación de Tesorero/Secretaria (hoy el Pastor los crea con password temporal) al mecanismo de invitación de Supabase Auth.
9. Actualizar el frontend (repo aparte) para usar `supabase-js` en vez de `POST /auth/login` / `POST /auth/refresh` actuales.

**Recomendación:** probar de punta a punta contra un proyecto Supabase de prueba (o el mismo proyecto pero en un ambiente separado) antes de cortar el sistema actual — no migrar en caliente contra usuarios reales de una iglesia sin haber validado 1:1 que las tres reglas del punto 4 siguen funcionando.
