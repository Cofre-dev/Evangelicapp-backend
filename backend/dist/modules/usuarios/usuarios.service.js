...
let UsuariosService = class UsuariosService {
    ...
    async findAllForIglesia(iglesiaId) {
        return this.prisma.usuario.findMany({
            where: { iglesiaId, rol: { not: client_1.Rol.PASTOR } },
    ...
    async update(iglesiaId, usuarioId, dto) {
    ...
    async findMiembroEquipoOrThrow(iglesiaId, usuarioId) {
        const usuario = await this.prisma.usuario.findFirst({
            where: { id: usuarioId, iglesiaId, rol: { not: client_1.Rol.PASTOR } },
            select: { id: true },
        });
        if (!usuario) {
